export async function getAllBook(params) {
    try{
        const res = await fetch(`https://nextjs-homework005.vercel.app/api/book${params ? `?query=${params}` : ""}`);
...
export async function getBookByCategory(params) {
    try{
        const res = await fetch(`https://nextjs-homework005.vercel.app/api/book?query=${params}`);
